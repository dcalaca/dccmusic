  ...
  return (
    <div className="min-h-screen py-6 sm:py-8"><div className="container mx-auto px-4 sm:px-6 lg:px-8"><div className="max-w-6xl mx-auto">
      ...
      <section id="compositor-premium" className="scroll-mt-24">
        <div className="mb-5"><h2 className="text-xl font-black sm:text-2xl">Planos para Compositor Premium</h2><p className="text-sm text-gray-400">Para publicar suas músicas e vídeos no DCC Music com página exclusiva.</p></div>
        {composerPlansWithPrices.length === 0 ? <div className="text-center py-16"><p className="text-gray-400 text-lg">Nenhum plano de compositor disponível no momento.</p></div> : <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">{composerPlansWithPrices.map(({ plan, priceQuote }) => {
          ...
          return <div key={plan.id} className={`relative flex h-full flex-col overflow-hidden rounded-2xl p-5 transition-all sm:p-8 ${goldPlan ? 'bg-gradient-to-br from-yellow-950/30 via-gray-900/70 to-purple-950/40 border border-yellow-500/60 hover:border-yellow-400 shadow-lg shadow-yellow-950/20' : 'bg-gray-900/50 border border-gray-800 hover:border-primary-500'}`}>
            {goldPlan && <div className="absolute right-4 top-4 rounded-full border border-yellow-400/50 bg-yellow-500/20 px-3 py-1 text-xs font-bold text-yellow-200">Plano Ouro</div>}
            <div className="mb-6"><h3 className="text-2xl font-bold mb-2">{plan.name}</h3>{priceQuote ? <div className="flex items-baseline flex-wrap" data-no-translate><span className="text-3xl font-bold text-primary-400">{formatCurrency(priceQuote.amount, priceQuote.currency)}</span><span className="text-gray-400 ml-2 text-sm">/{isUS ? (plan.durationMonths === 1 ? 'month' : plan.durationMonths === 12 ? 'year' : `${plan.durationMonths} months`) : (plan.durationMonths === 12 ? 'ano' : `${plan.durationMonths} meses`)}</span></div> : <p className="font-bold text-amber-300">Price unavailable</p>}{plan.description && <p className="text-gray-400 mt-2 text-sm">{plan.description}</p>}</div>
            ...
            {priceQuote ? <PlanPurchaseLink href={`/compositores/checkout?plan=${plan.slug}`} planType="composer" className="block w-full px-4 py-3 bg-gradient-to-r from-primary-600 to-purple-600 hover:from-primary-700 hover:to-purple-700 rounded-lg transition-all font-medium text-center mt-auto">Assinar Agora</PlanPurchaseLink> : <span className="mt-auto block w-full cursor-not-allowed rounded-lg bg-gray-800 px-4 py-3 text-center font-medium text-gray-500">Unavailable</span>}
          </div>
        })}</div>}
      </section>
    </div></div></div>
