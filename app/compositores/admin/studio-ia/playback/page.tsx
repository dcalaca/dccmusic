...
function getPlaybackError(t: TFunction, data: { errorCode?: string; creditsRequired?: number }, fallbackKey: string) {
  return data.errorCode
    ? t(`studio.tools.playback.errors.${data.errorCode}`, {
...
function PlaybackCreator() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        if (!response.ok) throw new Error(getPlaybackError(t, data, 'studio.tools.playback.errors.load'))
        ...
        if (!savedResponse.ok) throw new Error(getPlaybackError(t, savedData, 'studio.tools.playback.errors.loadSaved'))
  ...
  const uploadFile = async (file: File) => {
    ...
    try {
      ...
      if (!prepareResponse.ok) throw new Error(getPlaybackError(t, prepareData, 'studio.tools.playback.errors.prepareUpload'))
    ...
    try {
      ...
      if (!response.ok) {
        const refund = Number(data.creditsRefunded) > 0
          ? ` ${t('studio.tools.playback.errors.refunded', { count: data.creditsRefunded })}`
          : ''
        throw new Error(getPlaybackError(t, data, 'studio.tools.playback.errors.create') + refund)
      }
      setResult(data)
            ...
            {loading ? (
              ...
              <>
                <div className="rounded-2xl border border-white/10 bg-black/40 p-5">
                  ...
                  <p className="mt-1 text-sm text-gray-400">{t('studio.tools.playback.version')} {versionNumber}{version.versionName && getAutoGeneratedStudioVersionNumber(version.versionName) === null ? ` · ${version.versionName}` : ''}</p>
