...
function PlaybackCreator() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        if (!response.ok) throw new Error(data.error || 'Erro ao carregar a música.')
  ...
  const createPlayback = async () => {
    ...
    try {
      ...
      const data = await response.json()
      if (!response.ok) {
        const refund = Number(data.creditsRefunded) > 0 ? ' Seus 10 créditos foram devolvidos.' : ''
        throw new Error((data.error || 'Não foi possível criar o playback.') + refund)
      }
      setResult(data)
      window.dispatchEvent(new Event('studioBalanceChange'))
    } catch (createError: any) {
      setError(createError?.message || 'Não foi possível criar o playback.')
      window.dispatchEvent(new Event('studioBalanceChange'))
    } finally {
      setProcessing(false)
    }
      ...
      <div className="mx-auto max-w-3xl">
        ...
        <section className="overflow-hidden rounded-3xl border border-cyan-400/25 bg-gray-950/90 shadow-2xl shadow-cyan-950/30">
          ...
          <div className="space-y-5 p-5 sm:p-8">
            {loading ? (
              <div className="flex items-center gap-3 rounded-2xl border border-white/10 bg-black/40 p-5 text-gray-300"><FiLoader className="animate-spin" /> Carregando a versão escolhida...</div>
            ) : project && version ? (
              <>
                <div className="rounded-2xl border border-white/10 bg-black/40 p-5">
                  <p className="text-xs font-black uppercase tracking-wider text-cyan-300">Versão escolhida</p>
                  <h2 className="mt-1 text-xl font-black">{project.title || 'Minha música'}</h2>
                  <p className="mt-1 text-sm text-gray-400">Versão {versionNumber}{version.versionName ? ` · ${version.versionName}` : ''}</p>
...
export default function PlaybackPage() {
  return <Suspense fallback={<main className="min-h-screen bg-black p-8 text-white">Carregando...</main>}><PlaybackCreator /></Suspense>
}
