...
export default function EditComposerVideoPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      loadVideo(params.id, data.id)
    } catch (error) {
      router.push('/compositores/login')
    }
  }, [router, params.id])

  const loadVideo = async (videoId: string, composerId: string) => {
    try {
      const token = localStorage.getItem('composer_token')
      // Buscar o vídeo diretamente com compositores incluídos
      ...
      if (response.ok) {
        ...
        if (data.video) {
          console.log('[PAGE] Vídeo carregado com compositores:', {
            id: data.video.id,
            title: data.video.title,
            composers: data.video.composers?.map((c: any) => c.name) || []
          })
          setVideo(data.video)
        } else {
...
function ComposerVideoForm({ 
  ...
  const [selectedComposers, setSelectedComposers] = useState<string[]>(
    video.composers?.map((c: any) => c.name) || [composerName]
  )
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(`/api/compositores/videos/${video.id}`, {
        method: 'PUT',
        headers: {
