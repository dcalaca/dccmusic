...
type Result = { audio?: string; lyrics?: string; description?: string; timingPlan?: string; creativeDirection?: string; generatedLyrics?: boolean; error?: string; details?: string }
  ...
  async function generate(event: FormEvent) {
    ...
    try {
      const response = await fetch('/api/lab/lyria', {
        ...
        body: JSON.stringify({ ...form, style, lyrics: hasOwnLyric ? lyrics : '', bpm, duration, naturalProsody: true }),
      })
          ...
          <div className="min-w-0 space-y-5">
            ...
            <form onSubmit={generate} className="overflow-hidden rounded-[1.75rem] border border-white/10 bg-gray-950/85 p-3 shadow-2xl shadow-black/30 sm:p-5">
              <div className="grid gap-4 lg:grid-cols-2">
                ...
                <section className="rounded-[1.5rem] border border-primary-400/20 bg-primary-950/15 p-4 sm:p-5 lg:col-span-2"><SectionTitle icon={<FiActivity />} title="Engenharia musical DCC" subtitle="Andamento definido e duração planejada. Nunca usamos BPM automático." /><div className="mt-5 grid gap-3 sm:grid-cols-2"><Select label="Andamento obrigatório (BPM)" value={bpm} options={['90', '100', '110', '120']} onChange={setBpm} /><Select label="Duração planejada" value={duration} options={['120', '150', '180']} labels={{ '120': '2 minutos', '150': '2 minutos e 30 segundos', '180': '3 minutos' }} onChange={setDuration} /></div><p className="mt-3 text-xs text-purple-100/70">A DCC define intro, pausas, tempo de cada verso e finalização antes de enviar a letra ao Google.</p></section>
...
function formatDuration(seconds: string) {
  return `${Math.floor(Number(seconds) / 60)}:${String(Number(seconds) % 60).padStart(2, '0')}`
}
