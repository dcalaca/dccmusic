...
function shouldStartNewValidation(error: any) {
  const providerMessage = String(error?.providerMessage || '').toLowerCase()
  return Number(error?.providerCode) === 400 && (
    providerMessage.includes('record is not found') ||
    providerMessage.includes('does not need to be rebuilt') ||
    providerMessage.includes('does not require a retry')
  )
}

async function regenerateOrCreateValidation(voice: any, voiceUrl: string) {
  if (voice.validation_task_id) {
    try {
      return await regenerateSunoVoiceValidation(voice.validation_task_id)
    } catch (error: any) {
      // A completed/invalid task cannot be regenerated by Suno. Start only a new
      // validation phrase from the saved base audio; never submit an old verify audio.
      if (!shouldStartNewValidation(error)) throw error
    }
  }

  return createSunoVoiceValidation({
    voiceUrl,
