...
type StudioTopupTier = { maxMusicQuantity: number; unitPrice: number; label: string }
export type StudioPlanPriceQuote = { amount: number; currency: StudioTopupCurrency }
...
const COUNTRY_BASE_PRICE: Record<string, { currency: StudioTopupCurrency; amount: number }> = {
...
function roundLocalPrice(value: number, country: DccCountry) {
  ...
  if (code === 'PY' || code === 'CO') return Math.round(value / 100) * 100
  return Number(value.toFixed(2))
}

function buildCountryTiers(country: DccCountry): StudioTopupTier[] {
  const basePrice = COUNTRY_BASE_PRICE[String(country)]?.amount ?? BRAZIL_BASE_PRICE
  return STUDIO_TOPUP_TIERS.map((tier) => ({ ...tier, unitPrice: roundLocalPrice((tier.unitPrice / BRAZIL_BASE_PRICE) * basePrice, country) }))
}
...
export function getStudioTopupQuote(inputQuantity: number, country: DccCountry = 'BR'): StudioTopupQuote {
  ...
  return { musicQuantity, credits: musicQuantity * STUDIO_MUSIC_CREDITS, unitPrice: tier.unitPrice, totalPrice, tierLabel: tier.label, currency: getStudioTopupCurrency(country) }
}
