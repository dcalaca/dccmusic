...
export default function CommentsSection({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(t('comments.errors.comment'))
      }
      ...
      alert(error.message || t('comments.errors.comment'))
    } finally {
      setSubmitting(false)
    }
  }
  ...
  const handleReply = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        const error = await response.json()
        throw new Error(t('comments.errors.reply'))
      }
      ...
      alert(error.message || t('comments.errors.reply'))
    } finally {
      setSubmitting(false)
    }
  }
        ...
        <div className="mb-2 flex items-start justify-between gap-4">
          <div className="flex min-w-0 flex-1 items-center gap-2">
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="text-xs text-gray-500">{formatDate(comment.createdAt, i18n.language)}</div>
            </div>
          </div>
          {myId === comment.userId && (
            <button
              ...
              title={t('comments.delete')} aria-label={t('comments.delete')}
            >
