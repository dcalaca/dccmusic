...
function translateMexicanPriceTextNode(node: Node, value: string) {
  const pattern = /R\$\s*([\d.]+(?:,\d{1,2})?)/g; let lastIndex = 0; let replaced = false; const fragment = document.createDocumentFragment(); let match: RegExpExecArray | null
  while ((match = pattern.exec(value))) { const brl = Number(String(match[1]).replace(/\./g, '').replace(',', '.')); if (!Number.isFinite(brl)) continue; const before = value.slice(lastIndex, match.index); if (before) fragment.appendChild(document.createTextNode(before)); fragment.appendChild(document.createTextNode(formatLocalizedMoney(brl, 'MX'))); const suffix = document.createElement('span'); suffix.textContent = ' MXN'; suffix.setAttribute('data-no-translate', 'true'); suffix.style.fontSize = '0.44em'; suffix.style.fontWeight = '700'; suffix.style.opacity = '0.72'; suffix.style.verticalAlign = '0.22em'; suffix.style.whiteSpace = 'nowrap'; fragment.appendChild(suffix); lastIndex = pattern.lastIndex; replaced = true }
  if (!replaced) return false; const after = value.slice(lastIndex); if (after) fragment.appendChild(document.createTextNode(after)); node.parentNode?.replaceChild(fragment, node); return true
}
