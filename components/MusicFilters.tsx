...
export default function MusicFilters({ genres, currentParams }: MusicFiltersProps) {
  ...
  const FiltersContent = () => (
    <div className="space-y-6">
      {/* Busca */}
      <div>
        <label className="block text-sm font-medium mb-2">{t('musicFilters.search')}</label>
        <input
          type="text"
          value={busca}
          onChange={(e) => setBusca(e.target.value)}
          placeholder={t('musicFilters.placeholder')}
      ...
      <div>
        <div className="flex items-center justify-between mb-2">
          <label className="block text-sm font-medium">{t('musicFilters.genres')}</label>
          <button
      ...
      {/* Plataforma */}
      <div>
        <label className="block text-sm font-medium mb-2">{t('musicFilters.platform')}</label>
        <select
          value={plataforma}
          onChange={(e) => setPlataforma(e.target.value)}
          className="w-full px-4 py-2 bg-gray-800 border border-gray-700 rounded-lg focus:outline-none focus:border-primary-500"
        >
          <option value="">{t('musicFilters.all')}</option>
      ...
      {/* Ordem */}
      <div>
        <label className="block text-sm font-medium mb-2">{t('musicFilters.sort')}</label>
        <select
          value={ordem}
          onChange={(e) => setOrdem(e.target.value)}
          className="w-full px-4 py-2 bg-gray-800 border border-gray-700 rounded-lg focus:outline-none focus:border-primary-500"
        >
          <option value="mais-vistos">{t('musicFilters.mostViewed')}</option>
          <option value="recentes">{t('musicFilters.recent')}</option>
          <option value="antigos">{t('musicFilters.old')}</option>
          <option value="az">{t('musicFilters.az')}</option>
        </select>
      </div>
    ...
    <>
      ...
      >
        <span className="flex items-center space-x-2">
          ...
          <span>{t('musicFilters.filters')}</span>
        </span>
      ...
      <div className="hidden lg:block bg-gray-900/50 p-6 rounded-lg border border-gray-800">
        <h2 className="text-lg font-semibold mb-4">{t('musicFilters.filters')}</h2>
        <FiltersContent />
      </div>
        ...
        <div className="lg:hidden fixed inset-0 z-50 bg-black/80 backdrop-blur-sm">
          <div className="absolute right-0 top-0 h-full w-80 bg-gray-900 border-l border-gray-800 p-6 overflow-y-auto">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-xl font-semibold">{t('musicFilters.filters')}</h2>
              <button
                onClick={() => setIsMobileOpen(false)}
