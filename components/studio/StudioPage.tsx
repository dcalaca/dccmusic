...
import { FiDownload, FiLoader, FiSave } from 'react-icons/fi'
  ...
  return (
    ...
    <div className="-mx-0 flex h-[calc(100dvh-4.5rem)] min-h-[520px] flex-col overflow-hidden bg-[#0b0d10] text-white md:h-[calc(100dvh-5rem)]">
      <header className="flex shrink-0 items-center gap-3 border-b border-white/10 bg-[#0b0d10] px-3 py-2.5 sm:px-4">
        ...
        <div className="flex shrink-0 items-center gap-2">
          ...
          <button
            ...
            className="inline-flex items-center gap-1.5 rounded-lg bg-white px-2.5 py-1.5 text-xs font-semibold text-black transition hover:bg-zinc-200 disabled:cursor-not-allowed disabled:bg-zinc-700 disabled:text-zinc-400 sm:px-3"
          >
      ...
      {(message || error || projectUrl) && (
        <div className="shrink-0 border-b border-white/10 px-3 py-2 text-xs sm:px-4">
          {error && <p className="text-red-300">{error}</p>}
          {message && <p className="text-emerald-300">{message}</p>}
          {projectUrl && (
            <Link href={projectUrl} className="mt-1 inline-block text-sky-300 underline">
