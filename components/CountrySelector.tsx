...
function selectorCurrency(country: DccCountry) {
  return COUNTRY_CONFIG[String(country)].currency
}
      ...
      {open ? (
        <div role="menu" className="absolute right-0 top-12 z-[80] w-56 overflow-hidden rounded-2xl border border-gray-700 bg-gray-950 p-2 shadow-2xl shadow-black/70">
          ...
          {Object.keys(COUNTRY_CONFIG).map((rawCode) => {
            ...
            const typedCode = rawCode as DccCountry
            return (
              <button
                key={rawCode}
                type="button"
                role="menuitemradio"
                aria-checked={code === rawCode}
                onClick={() => {
                  setCountry(typedCode)
                  setOpen(false)
                }}
                className={`flex w-full items-center gap-3 rounded-xl px-3 py-3 text-left text-sm transition ${code === rawCode ? 'bg-primary-600 text-white' : 'text-gray-200 hover:bg-gray-800'}`}
              >
