-- ============================================
-- Função SQL para buscar ou criar assinatura RETORNANDO DETALHES COMPLETOS
-- Esta função bypassa RLS e retorna JSON com todos os dados da assinatura
-- ============================================

-- Criar função para buscar ou criar assinatura retornando JSON completo
CREATE OR REPLACE FUNCTION dccmusic_get_or_create_subscription_complete(
  p_composer_id UUID,
  p_plan_id UUID,
...
BEGIN
  ...
  -- Se encontrou, retornar como JSON
  IF v_subscription_record.id IS NOT NULL THEN
    RETURN row_to_json(v_subscription_record);
  END IF;
  ...
  IF v_subscription_id IS NULL THEN
    ...
    LIMIT 1;
  ELSE
    -- Buscar registro completo do ID criado
    SELECT * INTO v_subscription_record
    FROM dccmusic_subscriptions
...
GRANT EXECUTE ON FUNCTION dccmusic_get_or_create_subscription_complete(UUID, UUID, VARCHAR) TO authenticated;
GRANT EXECUTE ON FUNCTION dccmusic_get_or_create_subscription_complete(UUID, UUID, VARCHAR) TO anon;
...
FROM pg_proc
WHERE proname = 'dccmusic_get_or_create_subscription_complete';
